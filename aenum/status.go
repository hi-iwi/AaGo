...
const (
	...
	_auditFailedLowerLimit Status = UserDeleted // 审核中下限
	...
	_auditFailedUpperLimit Status = AuditFailed - 1 // 审核中上限

	_pendingLowerLimit Status = _auditFailedUpperLimit // 审核中下限
	...
	_pendingUpperLimit Status = PendingAudit - 1 // 审核中上限

	// 临时基准线
	_approvedLowerLimit Status = _pendingUpperLimit // 审核通过下限
...
func (s Status) IsApproved() bool {
	return s != 0 && s > _approvedLowerLimit
}
func (s Status) IsPendding() bool {
	return s > _pendingLowerLimit && s < _pendingUpperLimit
}
func (s Status) IsFailed() bool {
	return s > _auditFailedLowerLimit && s < _auditFailedUpperLimit
}
func (s Status) MeVisible() bool {
	return s > _approvedLowerLimit
}
func StmtPendding(field string) string {
	a := field + ">" + strconv.FormatInt(int64(_pendingLowerLimit), 10)
	b := field + "<" + strconv.FormatInt(int64(_pendingUpperLimit), 10)
	return a + " && " + b
}
func StmtFailed(field string) string {
	a := field + ">=" + strconv.FormatInt(int64(_auditFailedLowerLimit), 10)
	b := field + "<=" + strconv.FormatInt(int64(_auditFailedUpperLimit), 10)
	return a + " && " + b
}

// 自己可见的声明语句
func StmtMeVisible(field string) string {
	return field + ">" + strconv.FormatInt(int64(_approvedLowerLimit), 10)
}

// 自定义可见；如用户与作者互相关注，而且也被其分入特定用户组，那么可见语句为：
// s.StmtVisible("status", OnlyVisibleToFansOk, OnlyVisibleToFolloweeOk, OnlyVisibleToSpecificUsersOk)
func StmtVisible(field string, sts ...Status) string {
	stmt := field + ">" + strconv.FormatInt(int64(_approvedLowerLimit), 10)
	if len(sts) > 0 {
		for _, st := range sts {
